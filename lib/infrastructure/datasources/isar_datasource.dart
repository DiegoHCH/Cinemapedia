...
class IsarDatasource extends LocalStorageDatasource {
  ...
  Future<Isar> openDB() async {
    ...
    final directory = await getApplicationDocumentsDirectory();
      ...
      return await Isar.open([MovieEntitySchema], inspector: true, directory: directory.path);
    }
  ...
  Future<bool> isMovieFavorite(int movieId) async{
    ...
    final MovieEntity? isFavoriteMovie = await isar.movieEntitys
      .filter()
      .idEqualTo(movieId)
      .findFirst();
  ...
  Future<void> toggleFavorite(MovieEntity movie) async {
    ...
    final favoriteMovie = await isar.movieEntitys
      .filter()
      .idEqualTo(movie.id)
      .findFirst();
  ...
  Future<List<MovieEntity>> loadMovies({int limit = 10, offset = 0}) async {
    ...
    return isar.movieEntitys.where()
      .offset(offset)
      .limit(limit)
      .findAll();
